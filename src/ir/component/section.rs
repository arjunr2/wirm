    ...
    match ty {
        ComponentType::Component(decls) => {
            ...
            for (idx, decl) in decls.iter().enumerate() {
                populate_space_for_comp_ty_comp_decl(idx, &space_id, decl, &section, handle.clone());
            }
        ...
        ComponentType::Instance(decls) => {
            ...
            let section = ComponentSection::ComponentType(Some(space_id.clone()));
            for (idx, decl) in decls.iter().enumerate() {
                populate_space_for_comp_ty_inst_decl(idx, &space_id, decl, &section, handle.clone());
            }
...
fn populate_space_for_comp_ty_comp_decl(idx: usize, space_id: &SpaceId, decl: &ComponentTypeDeclaration, section: &ComponentSection, handle: StoreHandle) {
    let space = decl.index_space_of();
    handle.borrow_mut().assign_assumed_id(space_id, &space, section, idx);
}

fn populate_space_for_comp_ty_inst_decl(idx: usize, space_id: &SpaceId, decl: &InstanceTypeDeclaration, section: &ComponentSection, handle: StoreHandle) {
    let space = decl.index_space_of();
    handle.borrow_mut().assign_assumed_id(space_id, &space, section, idx);
}
...
fn populate_space_for_core_module_decl(idx: usize, space_id: &SpaceId, decl: &ModuleTypeDeclaration, section: &ComponentSection, handle: StoreHandle) {
    let space = decl.index_space_of();
    handle.borrow_mut().assign_assumed_id(space_id, &space, section, idx);
}
